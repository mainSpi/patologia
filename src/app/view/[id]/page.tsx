...
async function getCardData(id: string): Promise<CardData | null> {
  try {
    // In a real app, this would be an API call. For now, fetch from public JSON.
    // This fetch needs to be absolute for server-side rendering if your app is deployed.
    // For local dev, relative path works. For Vercel/Netlify, you need absolute or a proper API route.
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:9002'; // Fallback for local
    const res = await fetch(`${baseUrl}/data/cards.json`, { cache: 'no-store' }); // no-store for dev, consider revalidate for prod
    if (!res.ok) {
...
export default async function ViewPage({ params }: { params: { id: string } }) {
  ...
  // Ensure svsUrl is a valid URL for OpenSeadragon. 
  // For DZI, it might be a path to an XML file. For simple images, it's a direct image URL.
  // Example assumes svsUrl is a direct image URL suitable for OpenSeadragon's 'simpleImage' type.
...
// This function is needed if you are using generateStaticParams for SSG
// For dynamic SSR, it's not strictly required but good practice.
